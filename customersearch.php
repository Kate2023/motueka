<?php 
//provides JSON data to an Ajax client 
...
$searchresult = ''; 
if (isset($searchstring) and !empty($searchstring) and strlen($searchstring) < 31) 
{ 
    $searchstring = strtolower($searchstring); 
 
//prepare a query and send it to the server using our search string as a wildcard on surname 
    $query = "SELECT customerID, firstname, lastname FROM customer WHERE lastname LIKE '$searchstring%' ORDER BY lastname"; 
...
//append each row in the query result to our empty array until there are no more results                     
    ...
    { 
        echo "<tr><td colspan=3><h2>No Customers found!</h2></td></tr>"; 
    } 
} 
else 
{ 
